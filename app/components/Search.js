      ...
      <div className="search-overlay-bottom">
        <div className="container container--narrow py-3">
          ...
          <div className={"live-search-results " + (state.show == "results" ? "live-search-results--visible" : "")}>
            {Boolean(state.results.length) && (
              <div className="list-group shadow-sm">
                ...
                {state.results.map(post => {
                  ...
                  return (
                    <Link onClick={() => appDispatch({ type: "closeSearch" })} to={`/posts/${post._id}`} className="list-group-item list-group-item-action" key={post._id}>
            ...
            {!Boolean(state.results.length) && <p className="alert alert-danger text-center shadow-sm">No results found. Try again!</p>}
          </div>
        </div>
      </div>
    </div>
